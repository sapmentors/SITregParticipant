...
function IssuesHandler(sFullPath) {
    ...
    return {
        handleError: function (level, msg) {
            ...
            _text = _text.replace(/!!$/,""); //xmldom messages contains !!, which are not user friendly. removing it.
...
function _getViewFilesForProject(validationMetadata, managedResources){
    ...
    var fileResources = [];
        ...
        files.forEach(function (filePath) {
			if (filePath.endsWith(".view.xml") || filePath.endsWith(".fragment.xml") || managedPaths.indexOf(filePath) !== -1) {
...
module.exports = {
    validateFileForFioriChecks: function validateFileForFioriChecks(xmlParsedDoc, fileResource) {
        var report = {issues: []};
        fioriAnalysis.setAsFiori(fileResource);
        //run fiori rules and ui5 semantic checks
        try {
    ...
    executeCommand: function (commandId, validationMetadata, managedResources){
        if (commandId === GENERATE_IDS_COMMAND){
            ...
            let edits = fioriAnalysis.generateStableIds(fileResources);
